...
export default function DialogUpdatePelajar({
    open,
    onClose,
    currentData,
    onSuccess,
    ...
    const onSubmit = form.handleSubmit(async (values) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(result?.error || result?.message || "Gagal memperbarui pelajar");
            }

            toast.success("Data pelajar berhasil diperbarui");
            await onSuccess();
            onClose();
        } catch (error) {
            toast.error("Gagal memperbarui pelajar", {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
                <div className="mb-4 flex items-center gap-3">
                    ...
                    <h2 className="text-2xl font-medium text-foreground">Edit Pelajar</h2>
                </div>
