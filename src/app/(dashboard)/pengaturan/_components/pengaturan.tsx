...
function ThemeToggle() {
    ...
    useEffect(() => {
        const dark = localStorage.getItem("scodule-theme") === "dark";
    ...
    const toggle = () => {
        ...
        localStorage.setItem("scodule-theme", next ? "dark" : "light");
        setIsDark(next);
    };

    return <button type="button" onClick={toggle} className="rounded-lg px-3 py-2 text-sm text-slate-600 transition hover:bg-primary/10 hover:text-primary" aria-pressed={isDark}>{isDark ? "Mode terang" : "Mode gelap"}</button>;
}
...
export default function PengaturanPage() {
    ...
    return <section className="min-h-[calc(100vh-7rem)] px-4 py-6 sm:px-6 md:px-10 lg:px-12" style={{ backgroundColor: "var(--background)" }}>
        <div className="mx-auto max-w-6xl">
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium uppercase tracking-[0.16em] text-primary">Akun</p><h1 className="mt-2 text-2xl font-semibold text-slate-900 sm:text-3xl">Pengaturan</h1></div><ThemeToggle /></div>
            <div className="mb-5 inline-flex rounded-xl bg-slate-200 p-1 dark:bg-zinc-800">{(["profile", "security"] as const).map((item) => <button key={item} type="button" onClick={() => setTab(item)} className={`rounded-lg px-4 py-2 text-sm font-medium transition ${tab === item ? "bg-primary text-white" : "text-slate-600 hover:text-primary"}`}>{item === "profile" ? "Profile" : "Keamanan"}</button>)}</div>
